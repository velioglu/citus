...
ModifyQuerySupported(Query *queryTree, bool multiShardQuery)
{
	...
	if (queryTree->hasSubLinks == true)
	{
		...
		if ((commandType != CMD_UPDATE && commandType != CMD_DELETE) || multiShardQuery)
		{
				...
				DeferredErrorMessage **planningError)
{
	...
	RouterQuery(originalQuery, restrictionContext,
	...
	{
		RangeTblEntry *rangeTableEntry = (RangeTblEntry *) lfirst(rangeTableCell);
		bool requiresUpdateOrDeletePermissions = false;

		/* XXX: todo refactor into a function */
		if ((ACL_UPDATE & rangeTableEntry->requiredPerms) ||
			(ACL_DELETE & rangeTableEntry->requiredPerms))
		{
			requiresUpdateOrDeletePermissions = true;
		}

		if (rangeTableEntry->rtekind == RTE_SUBQUERY && requiresUpdateOrDeletePermissions)
		{
	...
	else
	{
		...
		foreach(rangeTableCell, rangeTableList)
		{
			...
			if (requiresUpdateOrDeletePermissions)
			{
				replicationModel = cacheEntry->replicationModel;
				modificationPartitionMethod = cacheEntry->partitionMethod;
			}
			else if (cacheEntry->partitionMethod != DISTRIBUTE_BY_NONE)
			{
				distributedSelect = true;
			}

			if (modificationPartitionMethod == DISTRIBUTE_BY_NONE && distributedSelect)
			{
 ...
 */
void
RouterQuery(Query *originalQuery, RelationRestrictionContext *restrictionContext,
